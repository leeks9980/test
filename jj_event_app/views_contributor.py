...
def HTMLtemplate(articletag ,id = None):
    ...
    ol = ''
    for topic in topics:
        ol += f'<li><a href="/read/{topic["id"]}">{topic["tittle"]}</a></li>'
    return f'''
    <html>
    <body>
        ...
        if topic['id'] == int(id):
            for location in event_location:
                if location['id'] == int(id):
                    article = f''''
                    <h2>{topic["tittle"]}</h2>
                    <img src="/static/image/{topic["event_poster"]}" alt="image"><br>
                    행사 위치<br>
...
def create(request):
    ...
    if request.method == 'GET':
        ...
        <form action = "/create/" method="post" enctype="multipart/form-data">
                ...
                <p><input type = "file" name ="image" placeholder="행사 포스터"></p>
...
def update(request,id):
    ...
    if request.method == 'GET':
        for topic in topics:
            if topic['id'] == int(id):
                selectedTopics = {
                    ...
                    "event_poster":topic["event_poster"]
                }
        ...
        if request.FILES['image'] != None:
            ...
            shutil.move(f"C:/python_workspace/{filename}","C:\python_workspace\static\image")
            image_lable += 1
        elif request.POST['image'] == '(binary)':
            for topic in topics:
                if topic['id'] == int(id):
                    filename = topic["event_poster"] 
        ...
        for topic in topics:
            if topic['id'] == int(id):
                topic['tittle'] = tittle
                topic['event_time'] = event_time
                topic['Event_Description'] = Event_Description
                topic['event_poster'] = filename  
...
def student_HTMLtemplate(articletag ,id = None):
    ...
    ol = ''
    for topic in topics:
        ol += f'<li><a href="/read_sutdent/{topic["id"]}">{topic["tittle"]}</a></li>'
    return f'''
    <html>
    <body>
